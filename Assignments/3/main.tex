...
\begin{enumerate}[itemsep=0pt,topsep=0pt,label=(\alph*)]
  ...
  \begin{enumerate}[itemsep=0pt,topsep=0pt,label=\roman*.]
    ...
    \begin{itemize}[itemsep=0pt,topsep=0pt]
      \item Parallel to the optical axis from the object, then it is refracted with direction to the fron focal length.
      \item Direct to the back focal length: it is refracted so that it becomes parallel to the optical axis.
      \item The chief ray, which maintain its direction through its propagation.
    \end{itemize}
    The intersection of these three rays producte the image. We can see that the image is to the left of the lens, but to the right of the object.
  ...
  \end{enumerate}
  \item The Galilean telescope is composed of a positive lens and a negative to the right (figure \ref{fig:1b}). In order to make the system afocal, 
  ...
  \begin{enumerate}[itemsep=0pt,topsep=0pt,label=\roman*.]
    ...
    We see that $f_1$ is the total distance, shich can be thought as the sum of $f_1$ and $t$. Then, equating them and solving gives the distance $t$:
    \begin{align*}
      f_1&=t-f_2\\
    ...
    before reaching the point the negative lens refracts again the ray so that it goes back to its initial direction parallel to the optical axis. 
...
\begin{enumerate}[itemsep=0pt,topsep=0pt,label=(\alph*)]
  ...
  \end{align} 
  \begin{enumerate}[itemsep=0pt,topsep=0pt,label=\roman*.]
    \item For a positive lens $f=+100\;mm$, we have figure \ref{fig:2a1}. We can see that the curve has two asymptotas which corresponds to:
    \begin{itemize}[itemsep=0pt,topsep=0pt]
    ...
    \end{itemize} 
    \item For $f=-100\;mm$, we have the figure \ref{fig:2a2}, where the same behavior is obtained as in th eprevious case. In this case,
  ...
  \begin{enumerate}[itemsep=0pt,topsep=0pt,label=\roman*.]
    ...
    \begin{itemize}[itemsep=0pt,topsep=0pt]
      \item\textbf{(1)} Real object located at $z=-1$, generating a real image with $m=\infty$.
      \item\textbf{(2)} Real image with $m=0$, which is due to an real object at $z=-\infty$.
    \end{itemize} 
    ...
    the asymptotes are displaced so that:
    \begin{itemize}[itemsep=0pt,topsep=0pt]
      \item\textbf{(1)} Virtual object located at $z=1$, generating a real image with $m=\infty$.
      \item\textbf{(2)} Virtual image located with $m=0$, which is due to a real object at $z=-\infty$. 
    \end{itemize} 
  \end{enumerate}
  \begin{figure}[h!]
    \centering
  ...
  \begin{enumerate}[itemsep=0pt,topsep=0pt,label=\roman*.]
    ...
    \end{align*}
    Thus, both the object and image are located to the left of the lens, the object is real but the image is virtual and to the left of the object.
    ...
    By t=doing an analogus procedure as before, we found that $z=f/2$ and $z'=f$. However, after verifying with a ray diagram we concluded that is \textbf{not possible} 
    to obtain a magnified image by either considering a real or virtual object. So, is not possible to meet a $+2$ magnified image even when the plot is 
