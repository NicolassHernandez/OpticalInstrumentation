...
\bfemph{Axial chromatic aberration} or \bfemph{axial color} is a variation of the system focal length with wavelength. This 
...
\begin{align*}
    \delta\phi_{FC}&=\phi_F-\phi_C=(n_F-1)(C_1-C_2)-(n_C-1)(C_1-C_2)=(n_F-n_C)(C_1-C_2)\\
    \delta\phi_{FC}&=\underbrace{\frac{n_F-n_C}{n_d-1}}_{1/\nu}\underbrace{(n_d-1)(C_1-C_2)}_{\phi_d}=\highlight{\frac{\phi_d}{\nu}=\delta\phi_{FC}}.
\end{align*}
Similarly, for the focal length:
\begin{align*}
    \delta f_{CF}=f_C-f_F=\frac{1}{\phi_C}-\frac{1}{\phi_F}=\frac{\phi_F-\phi_C}{\phi_C\phi_F}=\frac{\delta\phi}{\phi_C\phi_F}\approx\frac{\delta\phi}{\phi_d^2}=\frac{\phi_d}{\nu\phi_d^2}=\highlight{\frac{f_d}{\nu}=\delta f_{CF}}.
\end{align*}
\end{emphasizer}
...
The thin lens \bfemph{achromatic doublet} corrects longitudinal chromatic aberration by combining a positive element with a negative one.
...
Large differences in the Abbe number minimize the excess power and provide better performance.
\begin{figure}[h!]
    \centering
    \includegraphics[width=.3\columnwidth]{PartOne/ChapterThree/figures/excesspower.png}
...
{\small
\begin{align*}
    \delta\phi_{dC1}=(n_{d1}-n_{C1})(C_1-C_2)=\frac{(n_{d1}-n_{C1})}{(n_{F1}-n_{C1})}(n_{F1}-n_{C1})(C_1-C_2)=P_1\delta\phi_{FC1}.
\end{align*}}
Similarly,
\begin{align*}
    \delta\phi_{dC1}=P\delta\phi_{FC1},\quad\delta\phi_{dC2}=P_2\delta\phi_{FC2},\quad P_1=\frac{n_{d1}-n_{C1}}{n_{F1}-n_{C1}},\quad P_2=\frac{n_{d2}-n_{C2}}{n_{F2}-n_{C2}}.
\end{align*}
...
\begin{align*}
    ...
    &=P_1\frac{\phi_1}{\nu_1}+P_2\frac{\phi_2}{\nu_2}\\
    \delta\phi_{dC}&=\frac{P_1-P_2}{\nu_1-\nu_2}\phi\quad\text{Achromat eq: }\frac{\phi_1}{\nu_1}=-\frac{\phi_2}{\nu_2}.
\end{align*}
Then, the secondary deviation is:
\begin{align*}
    \text{Secondary chromatic aberration}\qquad\highlight{\delta\phi_{dC}=\frac{\Delta P}{\Delta \nu}\phi}.
\end{align*}
Consequently, in order to obtain zero secondary achromatic aberration with a doublet, the partial dispersion ratio $P$ of the two glasses \textbf{must be zero}:
\begin{align}
